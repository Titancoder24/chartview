...
export default function WaffleChartComponent({ data }: { data: { name: string; value: number }[] }) {
  ...
  data.forEach((d, i) => {
    ...
    for (let j = 0; j < count && cells.length < 100; j++) cells.push(theme.colors[i % theme.colors.length]);
  });
  ...
  return (
    <ChartCard title="Waffle Chart" subtitle="Proportional grid visualization">
      <div className="grid grid-cols-10 gap-1">
        {cells.map((color, i) => (
          <div key={i} className="aspect-square rounded-[3px] transition-all hover:scale-110" style={{ backgroundColor: color }} />
        ))}
      </div>
