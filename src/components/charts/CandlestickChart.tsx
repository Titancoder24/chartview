...
export default function CandlestickChartComponent({ data }: { data: Candle[] }) {
  ...
  const allVals = data.flatMap(d => [d.high, d.low]); const minVal = Math.min(...allVals); const maxVal = Math.max(...allVals); const range = maxVal - minVal || 1;
    ...
    <ChartCard title="Stock Price" subtitle="OHLC candlestick chart">
      <svg width="100%" height={svgH + 25} viewBox={`0 0 ${svgW} ${svgH + 25}`} preserveAspectRatio="xMidYMid meet">
        {data.map((d, i) => {
          ...
          const bull = d.close >= d.open; const color = bull ? theme.positive : theme.negative;
          const bodyTop = y(Math.max(d.open, d.close)); const bodyH = Math.max(Math.abs(y(d.open) - y(d.close)), 1);
          return (
            <g key={i}>
              <line x1={x + candleW / 2} y1={y(d.high)} x2={x + candleW / 2} y2={y(d.low)} stroke={color} strokeWidth={1.5} />
              <rect x={x} y={bodyTop} width={candleW} height={bodyH} rx={2} fill={color} fillOpacity={bull ? 0.3 : 0.8} stroke={color} strokeWidth={1} />
