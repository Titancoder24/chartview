...
export default function ParallelCoordinatesComponent() {
  ...
  const svgW = 440, svgH = 200, pad = 40;
    ...
    <ChartCard title="Parallel Coordinates" subtitle="Multi-dimensional product comparison">
      <svg width="100%" height={svgH + 20} viewBox={`0 0 ${svgW} ${svgH + 20}`} preserveAspectRatio="xMidYMid meet">
        ...
        {items.map((item, i) => {
          ...
          return <polyline key={i} points={points} fill="none" stroke={theme.colors[i % theme.colors.length]} strokeWidth={1.5} strokeOpacity={0.6} strokeLinecap="round" />;
        })}
        {items.map((item, i) => item.values.map((v, j) => <circle key={`${i}-${j}`} cx={pad + j * axisGap} cy={yScale(v)} r={3} fill={theme.colors[i % theme.colors.length]} />))}
      </svg>
      <div className="flex gap-4 mt-2 justify-center">
        {items.map((item, i) => (
          <div key={i} className="flex items-center gap-1.5">
            <div className="w-2.5 h-2.5 rounded-full" style={{ backgroundColor: theme.colors[i % theme.colors.length] }} />
            <span className="text-xs" style={{ color: theme.textSecondary }}>{item.name}</span>
          </div>
        ))}
      </div>
    </ChartCard>
