...
export default function BarChartComponent({ data }: { data: any[] }) {
  ...
  return (
    <ChartCard title="Product Sales" subtitle="Sales by product category">
      <ResponsiveContainer width="100%" height={240}>
        <ReBarChart data={data}>
          <CartesianGrid strokeDasharray="3 3" stroke={theme.gridColor} />
          <XAxis dataKey="product" tick={{ fill: theme.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
          <YAxis tick={{ fill: theme.textMuted, fontSize: 11 }} axisLine={false} tickLine={false} />
          <Tooltip {...tooltip} />
          <Bar dataKey="sales" fill={theme.colors[0]} radius={[4, 4, 0, 0]} />
        </ReBarChart>
      </ResponsiveContainer>
    </ChartCard>
  );
