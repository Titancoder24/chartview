          ...
          <tbody>
            {data.map((row, i) => (
              <tr key={i}>
                ...
                {weeks.map((w, j) => {
                  const key = w.toLowerCase().replace('w', 'w');
                  const val = row[key] ?? 0;
                  return (
                    <td key={j} className="text-center py-1 px-2">
                      <div className="rounded-md py-1" style={{ backgroundColor: getCellBg(val) }}>
                        <span style={{ color: val > 60 ? '#fff' : theme.textPrimary }}>{val}%</span>
                      </div>
                    </td>
                  );
                })}
